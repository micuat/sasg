...
void main() {
  ...
  vec2 noiseFrag = fragCoord;
  noiseFrag.x -= delta * _noise(vec3(fragCoord.t * 100, 0.5, time * 10));
  if(noiseFrag.x < 0) noiseFrag.x += 1.0;
  if(noiseFrag.x > 1) noiseFrag.x -= 1.0;
	vec4 c1 = texture2D(texture, noiseFrag);
